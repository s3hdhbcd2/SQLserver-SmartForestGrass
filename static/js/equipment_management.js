...
function loadDevices() {
    ...
    fetch(url)
        ...
        .then(data => {
            ...
            if (!Array.isArray(data) || data.length === 0) {
                // 根据用户角色动态调整colspan
                const isAdmin = getCurrentUser().Role === '管理员' || getCurrentUser().Role === '系统管理员';
                const colspan = isAdmin ? 7 : 8;
                tableBody.innerHTML = `<tr><td colspan="${colspan}" style="text-align: center; color: #999;">未找到匹配的设备档案</td></tr>`;
                return;
            }
...
function loadMaintenanceRecords() {
    ...
    .then(data => {
        ...
        data.forEach(inspection => {
            ...
            } else if (auditStatus === '通过' || auditStatus === '已审核') {
        ...
        if (table) {
            ...
            const bodyCells = table.querySelectorAll('tbody td:nth-child(11)');
            
            if (hasOperationButtons) {
                // 显示操作列
                if (headerCells.length > 10) {
                    headerCells[10].style.display = '';
                }
