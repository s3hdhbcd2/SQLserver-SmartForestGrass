...
-- 按外键依赖顺序删除数据，使用IF EXISTS检查表是否存在

-- 删除Feedback表数据（如果表存在）
IF EXISTS (SELECT * FROM sys.tables WHERE name = 'Feedback' AND type = 'U')
BEGIN
    DELETE FROM Feedback;
END;

-- 删除GeneratedReport表数据（如果表存在）
IF EXISTS (SELECT * FROM sys.tables WHERE name = 'GeneratedReport' AND type = 'U')
BEGIN
    DELETE FROM GeneratedReport;
END;

-- 删除NotificationRecord表数据（如果表存在）
IF EXISTS (SELECT * FROM sys.tables WHERE name = 'NotificationRecord' AND type = 'U')
BEGIN
    DELETE FROM NotificationRecord;
END;

-- 删除EquipmentInspection表数据（如果表存在）
IF EXISTS (SELECT * FROM sys.tables WHERE name = 'EquipmentInspection' AND type = 'U')
BEGIN
    DELETE FROM EquipmentInspection;
END;

-- 删除MaintenanceRecord表数据（如果表存在）
IF EXISTS (SELECT * FROM sys.tables WHERE name = 'MaintenanceRecord' AND type = 'U')
BEGIN
    DELETE FROM MaintenanceRecord;
END;

-- 删除DeviceStatus表数据（如果表存在）
IF EXISTS (SELECT * FROM sys.tables WHERE name = 'DeviceStatus' AND type = 'U')
BEGIN
    DELETE FROM DeviceStatus;
END;

-- 删除WarningRecord表数据（如果表存在）
IF EXISTS (SELECT * FROM sys.tables WHERE name = 'WarningRecord' AND type = 'U')
BEGIN
    DELETE FROM WarningRecord;
END;

-- 删除ResourceChangeRecord表数据（如果表存在）
IF EXISTS (SELECT * FROM sys.tables WHERE name = 'ResourceChangeRecord' AND type = 'U')
BEGIN
    DELETE FROM ResourceChangeRecord;
END;

-- 删除MonitoringData表数据（如果表存在）
IF EXISTS (SELECT * FROM sys.tables WHERE name = 'MonitoringData' AND type = 'U')
BEGIN
    DELETE FROM MonitoringData;
END;

-- 删除ForestResource表数据（如果表存在）
IF EXISTS (SELECT * FROM sys.tables WHERE name = 'ForestResource' AND type = 'U')
BEGIN
    DELETE FROM ForestResource;
END;

-- 删除DeviceArchive表数据（如果表存在）
IF EXISTS (SELECT * FROM sys.tables WHERE name = 'DeviceArchive' AND type = 'U')
BEGIN
    DELETE FROM DeviceArchive;
END;

-- 删除Sensor表数据（如果表存在）
IF EXISTS (SELECT * FROM sys.tables WHERE name = 'Sensor' AND type = 'U')
BEGIN
    DELETE FROM Sensor;
END;

-- 删除WarningRule表数据（如果表存在）
IF EXISTS (SELECT * FROM sys.tables WHERE name = 'WarningRule' AND type = 'U')
BEGIN
    DELETE FROM WarningRule;
END;

-- 删除ReportTemplate表数据（如果表存在）
IF EXISTS (SELECT * FROM sys.tables WHERE name = 'ReportTemplate' AND type = 'U')
BEGIN
    DELETE FROM ReportTemplate;
END;

-- 删除Region表数据（如果表存在）
IF EXISTS (SELECT * FROM sys.tables WHERE name = 'Region' AND type = 'U')
BEGIN
    DELETE FROM Region;
END;

-- 删除User表数据（如果表存在）
IF EXISTS (SELECT * FROM sys.tables WHERE name = 'User' AND type = 'U')
BEGIN
    DELETE FROM [User];
END;
...
('D008', '摄像头', '摄像头', 'CAM-001', '2023-01-08', 'R001', 'U001', 3),
('D009', '摄像头', '摄像头', 'CAM-002', '2023-01-09', 'R001', 'U001', 3),
('D010', '数据采集器', '传感器', 'COL-001', '2023-01-10', 'R001', 'U001', 3),
...
('D018', '摄像头', '摄像头', 'CAM-001', '2023-01-18', 'R002', 'U001', 3),
('D019', '摄像头', '摄像头', 'CAM-002', '2023-01-19', 'R002', 'U001', 3),
('D020', '数据采集器', '传感器', 'COL-001', '2023-01-20', 'R002', 'U001', 3),
('D021', '火灾预警器', '预警器', 'ALARM-001', '2023-02-01', 'R001', 'U001', 3),
('D022', '火灾预警器', '预警器', 'ALARM-001', '2023-02-02', 'R002', 'U001', 3),
('D023', '病虫害预警器', '预警器', 'ALARM-002', '2023-02-03', 'R003', 'U001', 3),
('D024', '病虫害预警器', '预警器', 'ALARM-002', '2023-02-04', 'R004', 'U001', 3),
('D025', '火灾预警器', '预警器', 'ALARM-001', '2023-02-05', 'R005', 'U001', 3);
...
INSERT INTO ReportTemplate (TemplateID, ReportName, ReportType, StatisticalIndicators, Description, StatisticalDimension, GenerationCycle)
VALUES
('TEMP001', '环境监测日报', '环境监测', '{"temperature": true, "humidity": true, "wind_speed": true, "rainfall": true, "smoke": true}', '每日环境监测数据汇总', '区域/时间', '日'),
('TEMP002', '环境监测周报', '环境监测', '{"temperature": true, "humidity": true, "wind_speed": true, "rainfall": true, "smoke": true}', '每周环境监测数据汇总', '区域/时间', '周'),
('TEMP003', '环境监测月报', '环境监测', '{"temperature": true, "humidity": true, "wind_speed": true, "rainfall": true, "smoke": true}', '每月环境监测数据汇总', '区域/时间', '月'),
('TEMP004', '设备状态日报', '设备状态', '{"running_status": true, "battery_level": true, "signal_strength": true}', '每日设备状态汇总', '设备/时间', '日'),
('TEMP005', '设备状态周报', '设备状态', '{"running_status": true, "battery_level": true, "signal_strength": true}', '每周设备状态汇总', '设备/时间', '周'),
('TEMP006', '设备状态月报', '设备状态', '{"running_status": true, "battery_level": true, "signal_strength": true}', '每月设备状态汇总', '设备/时间', '月'),
('TEMP007', '资源统计月报', '资源统计', '{"coverage_area": true, "tree_species": true, "growth_status": true}', '每月资源统计汇总', '区域/资源类型', '月'),
('TEMP008', '资源统计季报', '资源统计', '{"coverage_area": true, "tree_species": true, "growth_status": true}', '每季资源统计汇总', '区域/资源类型', '季'),
('TEMP009', '资源统计年报', '资源统计', '{"coverage_area": true, "tree_species": true, "growth_status": true}', '每年资源统计汇总', '区域/资源类型', '年'),
('TEMP010', '预警记录日报', '预警记录', '{"warning_type": true, "warning_level": true, "status": true}', '每日预警记录汇总', '区域/时间', '日'),
('TEMP011', '预警记录周报', '预警记录', '{"warning_type": true, "warning_level": true, "status": true}', '每周预警记录汇总', '区域/时间', '周'),
('TEMP012', '预警记录月报', '预警记录', '{"warning_type": true, "warning_level": true, "status": true}', '每月预警记录汇总', '区域/时间', '月'),
('TEMP013', '火灾风险评估', '环境监测', '{"temperature": true, "humidity": true, "smoke": true}', '火灾风险评估报告', '区域/时间', '月'),
('TEMP014', '旱情风险评估', '环境监测', '{"humidity": true, "rainfall": true}', '旱情风险评估报告', '区域/时间', '月'),
('TEMP015', '病虫害风险评估', '环境监测', '{"pest_disease": true}', '病虫害风险评估报告', '区域/时间', '月'),
('TEMP016', '设备维护建议', '设备状态', '{"running_status": true, "battery_level": true}', '设备维护建议报告', '设备/时间', '月'),
('TEMP017', '资源变化分析', '资源统计', '{"coverage_area": true, "change_type": true}', '资源变化分析报告', '区域/资源类型', '月'),
('TEMP018', '区域环境综合评估', '环境监测', '{"temperature": true, "humidity": true, "wind_speed": true, "rainfall": true, "smoke": true}', '区域环境综合评估报告', '区域', '月'),
('TEMP019', '年度环境总结', '环境监测', '{"temperature": true, "humidity": true, "wind_speed": true, "rainfall": true, "smoke": true}', '年度环境总结报告', '区域', '年'),
('TEMP020', '年度设备总结', '设备状态', '{"running_status": true, "battery_level": true, "signal_strength": true}', '年度设备总结报告', '设备', '年');
...
WHILE @m <= 20
BEGIN
    INSERT INTO GeneratedReport (ReportID, ReportName, TemplateID, RegionID, StartTime, EndTime, ReportContent, ReportFilePath, GeneratedTime, GeneratedBy, StatisticalCycle, DataSourceDescription)
    VALUES
    (
...
WHILE @q <= 25
BEGIN
    INSERT INTO Feedback (FeedbackID, FeedbackType, RegionID, Description, Contact, Status, HandlerID, HandleTime, HandleResult)
    VALUES
    (
        'FB' + RIGHT('0000' + CAST(@q AS VARCHAR(4)), 4),
        CASE WHEN @q % 5 = 0 THEN '火灾' WHEN @q % 5 = 1 THEN '病虫害' WHEN @q % 5 = 2 THEN '旱情' WHEN @q % 5 = 3 THEN '设备故障' ELSE '其他' END,
        'R' + RIGHT('00' + CAST(((@q - 1) % 20 + 1) AS VARCHAR(2)), 3),
        '反馈内容' + CAST(@q AS VARCHAR(4)) + ': ' + CASE WHEN @q % 5 = 0 THEN '发现火灾隐患' WHEN @q % 5 = 1 THEN '发现病虫害' WHEN @q % 5 = 2 THEN '发现旱情' WHEN @q % 5 = 3 THEN '设备故障' ELSE '其他反馈' END,
        'contact' + CAST(@q AS VARCHAR(4)) + '@example.com',
        CASE WHEN @q % 4 = 0 THEN '已处理' WHEN @q % 4 = 1 THEN '处理中' ELSE '待处理' END,
        CASE WHEN @q % 4 = 0 THEN 'U' + RIGHT('00' + CAST(((@q - 1) % 25 + 1) AS VARCHAR(2)), 3) ELSE NULL END,
        CASE WHEN @q % 4 = 0 THEN DATEADD(HOUR, -@q, GETDATE()) ELSE NULL END,
        CASE WHEN @q % 4 = 0 THEN '已处理，处理结果：' + CASE WHEN @q % 5 = 0 THEN '已扑灭火灾' WHEN @q % 5 = 1 THEN '已处理病虫害' WHEN @q % 5 = 2 THEN '已浇水抗旱' WHEN @q % 5 = 3 THEN '已修复设备' ELSE '已处理其他问题' END ELSE NULL END
    );
    SET @q = @q + 1;
END
