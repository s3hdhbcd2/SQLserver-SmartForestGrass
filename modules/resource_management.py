...
class ResourceManagementModule(BaseModule):
    ...
    def add_forest_resource(self, region_id, resource_type, coverage_area, tree_species, growth_status, updated_by):
        ...
        """
        try:
            resource_id = self.generate_id("FR")
            update_time = datetime.datetime.now()
            
            sql = """
            INSERT INTO ForestResource (ResourceID, RegionID, ResourceType, CoverageArea, TreeSpecies, GrowthStatus, PlantingTime, UpdateTime, UpdatedBy)
            VALUES (?, ?, ?, ?, ?, ?, GETDATE(), GETDATE(), ?)
            """
            params = (resource_id, region_id, resource_type, coverage_area, tree_species, growth_status, updated_by)
        ...
        try:
            ...
            for result in results:
                ...
                resources.append({
                    ...
                    'TreeSpecies': result[3],
                    'Quantity': result[4],
                    'CoverageArea': result[5],
                    'GrowthStatus': result[6],
                    'PlantingTime': result[7],
                    'UpdateTime': result[8],
                    'UpdatedBy': result[9]
                })
            return resources
        ...
        try:
            ...
            for result in results:
                ...
                change_records.append({
                    ...
                    'ResourceID': result[1],
                    'ChangeType': result[2],
                    'ChangeReason': result[3],
                    'ChangeTime': result[4],
                    'OperatorID': result[5],
                    'AuditStatus': result[6],
                    'AuditorID': result[7],
                    'AuditTime': result[8],
                    'AuditComments': result[9]
                })
    ...
    def audit_resource_change(self, change_id, audit_status, auditor_id, audit_comments=None):
        ...
        try:
            sql = """
            UPDATE ResourceChangeRecord 
            SET AuditStatus = ?, AuditorID = ?, AuditTime = GETDATE(), AuditComments = ?
            WHERE ChangeID = ?
            """
            params = (audit_status, auditor_id, audit_comments, change_id)
            return self.db.execute(sql, params)
        except Exception as e:
