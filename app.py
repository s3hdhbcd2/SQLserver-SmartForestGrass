...
def init_db():
    ...
    try:
        # 定义需要添加的用户列表
        users = [
            {'username': 'admin', 'password': '123456', 'name': '管理员', 'contact': 'admin@example.com', 'role': '系统管理员', 'status': '启用'},
    ...
    try:
        # 检查是否已有报表模板
        check_sql = "SELECT COUNT(*) FROM ReportTemplate"
        count_result = db_conn.fetch_one(check_sql)
        if count_result and count_result[0] == 0:
            # 插入默认报表模板
            templates = [
                ('TEMP001', '环境监测日报', '环境监测', '{"temperature": true, "humidity": true, "wind_speed": true, "rainfall": true, "smoke": true}', '每日环境监测数据汇总', '待审核'),
                ('TEMP002', '设备状态日报', '设备状态', '{"running_status": true, "battery_level": true, "signal_strength": true}', '每日设备状态汇总', '待审核'),
                ('TEMP003', '资源统计月报', '资源统计', '{"coverage_area": true, "tree_species": true, "growth_status": true}', '每月资源统计汇总', '待审核')
            ]
            
            for template in templates:
                template_id, report_name, report_type, indicators, description, audit_status = template
                sql = "INSERT INTO ReportTemplate (TemplateID, ReportName, ReportType, StatisticalIndicators, Description, AuditStatus, IsActive, CreateTime) VALUES (?, ?, ?, ?, ?, ?, 1, GETDATE())"
                try:
                    db_conn.execute(sql, (template_id, report_name, report_type, indicators, description, audit_status))
                    print(f"成功插入报表模板: {report_name}")
                except Exception as e:
                    print(f"插入报表模板失败: {report_name}, 错误: {e}")
            print("初始化报表模板数据成功")
        else:
            print(f"已有 {count_result[0]} 个报表模板，跳过初始化")
    except Exception as e:
        print(f"初始化报表模板数据失败: {e}")
...
def get_menu():
    ...
    # 统一使用系统管理员角色 - 系统管理员和管理员不需要环境监测和资源管理
    if username == 'admin' or role in ['系统管理员', '管理员', 'admin', 'system_admin']:
        menu = [
    ...
    menus = {
        ...
        '区域护林员': [
            ...
            {'code': '3', 'name': '资源管理', 'icon': '🌲', 'href': '/resource_management'},
            {'code': '4', 'name': '设备管理', 'icon': '🔧', 'href': '/equipment_management'},
            {'code': '5', 'name': '统计分析', 'icon': '📊', 'href': '/statistical_analysis'}  # 添加统计分析菜单
        ],
        '公众用户': [
            {'code': '1', 'name': '环境监测', 'icon': '🌡️', 'href': '/environmental_monitoring'},
            {'code': '5', 'name': '统计分析', 'icon': '📊', 'href': '/statistical_analysis'}
...
def resource_management():
    ...
    # 检查权限：数据管理员、监管人员和区域护林员可以访问资源管理
    role = session['user']['Role']
    if role not in ['数据管理员', '监管人员', '区域护林员']:
        return redirect(url_for('dashboard'))
...
def equipment_management():
    ...
    # 检查权限：系统管理员、管理员、区域护林员和监管人员可以访问设备管理
    role = session['user']['Role']
    if role not in ['系统管理员', '管理员', '区域护林员', '监管人员']:
        return redirect(url_for('dashboard'))
    from datetime import datetime
    now = datetime.now()
...
def publish_warning():
    ...
    # 验证用户角色：监管人员不能发布预警信息
    if role == '监管人员':
        return jsonify({'error': '监管人员不能发布预警信息', 'success': False}), 403
...
@app.route('/api/disaster_warning/supervise_warning', methods=['POST'])
def supervise_warning():
    if 'user' not in session:
        return jsonify({'error': '未登录', 'success': False}), 401
    ...
    # 检查权限：只有监管人员可以监督预警处理
    if role != '监管人员':
        return jsonify({'error': '没有权限监督预警处理', 'success': False}), 403
    ...
    try:
        ...
        if not warning_id or not evaluation or not comments:
            return jsonify({'error': '缺少必要参数', 'success': False}), 400
        
        # 这里应该调用业务层的监督方法，但目前业务层可能还没有实现这个方法
        # 暂时返回成功，后续可以添加具体的业务逻辑
        print(f"收到预警监督请求: 预警ID={warning_id}, 评价={evaluation}, 意见={comments}, 监督人ID={user_id}")
        
        # 这里可以添加具体的业务逻辑，比如保存监督记录到数据库
        # success = db_instance.supervise_warning(warning_id, evaluation, user_id, comments)
...
def get_resources():
    ...
    # 检查权限：系统管理员、数据管理员、区域护林员和监管人员可以访问资源列表API
    if role not in ['系统管理员', '数据管理员', '区域护林员', '监管人员']:
...
# 公开林草资源数据API（供公众用户访问）
@app.route('/api/resource_management/get_public_resources')
def get_public_resources():
    if 'user' not in session:
        return jsonify({'error': '未登录'}), 401
    
    # 所有登录用户都可以访问公开的林草资源数据
    region_id = request.args.get('region_id')
    resources = db_instance.get_forest_resources(region_id)
    return jsonify(resources)
...
def get_resource_changes():
    ...
    # 检查权限：系统管理员、数据管理员、监管人员和区域护林员可以访问资源变动记录API
    if role not in ['系统管理员', '数据管理员', '监管人员', '区域护林员']:
        return jsonify({'error': '没有权限访问该API', 'success': False}), 403
...
def get_regions():
    ...
    # 检查权限：系统管理员、管理员、数据管理员、区域护林员和监管人员可以访问区域列表API
    if role not in ['系统管理员', '管理员', '数据管理员', '区域护林员', '监管人员']:
        return jsonify({'error': '没有权限访问该API', 'success': False}), 403
    
    user_id = user['UserID']
    
    try:
        # 使用资源管理模块的独立数据库连接，避免全局连接繁忙
        db_conn = db_instance.resource_module.db
        ...
        # 使用资源管理模块的数据库连接执行查询
        results = db_conn.fetch_all(sql, params)
...
def add_resource():
    ...
    # 检查权限：只有系统管理员和数据管理员可以添加资源
    if role not in ['系统管理员', '数据管理员']:
        return jsonify({'error': '没有权限添加资源', 'success': False}), 403
...
def update_resource():
    ...
    # 检查权限：只有系统管理员和数据管理员可以更新资源
    if role not in ['系统管理员', '数据管理员']:
        return jsonify({'error': '没有权限更新资源', 'success': False}), 403
...
@app.route('/api/resource_management/audit_resource_change', methods=['POST'])
def audit_resource_change():
    ...
    # 检查权限：只有监管人员可以审核资源变动
    if role != '监管人员':
        return jsonify({'error': '没有权限审核资源变动', 'success': False}), 403
    ...
    try:
        ...
        if not change_id or not audit_status:
            return jsonify({'error': '缺少必要参数', 'success': False}), 400
        
        success = db_instance.audit_resource_change(change_id, audit_status, user_id, audit_comments)
        return jsonify({'success': success})
    except Exception as e:
        print(f"审核资源变动失败: {e}")
        return jsonify({'success': False, 'error': str(e)}), 500

@app.route('/api/resource_management/add_resource_change', methods=['POST'])
def add_resource_change():
    ...
    # 检查权限：系统管理员、监管人员和区域护林员可以添加资源变动记录
    if role not in ['系统管理员', '监管人员', '区域护林员']:
        return jsonify({'error': '没有权限添加资源变动记录', 'success': False}), 403
    ...
    try:
        ...
        if not resource_id or not change_type or not change_reason:
            return jsonify({'error': '缺少必要参数', 'success': False}), 400
        
        # 对于区域护林员，需要检查该资源是否属于其负责的区域
        if role == '区域护林员':
            # 查询该资源所属的区域
            region_sql = """
            SELECT r.RegionID FROM ForestResource f
            ...
            WHERE f.ResourceID = ? AND r.ManagerID = ?
            """
            result = db_instance.db.fetch_one(region_sql, (resource_id, user_id))
            if not result:
                return jsonify({'error': '该资源不属于您负责的区域', 'success': False}), 403
        
        # 调用添加资源变动记录的方法
        change_id = db_instance.add_resource_change_record(resource_id, change_type, change_reason, None, user_id)
        
        if change_id:
            return jsonify({'success': True, 'change_id': change_id})
        else:
            return jsonify({'success': False, 'error': '添加资源变动记录失败'})
    except Exception as e:
...
def get_devices():
    try:
        ...
        # 检查权限：系统管理员、管理员、区域护林员和监管人员可以访问设备管理API
        if role not in ['系统管理员', '管理员', '区域护林员', '监管人员']:
...
def get_device_status():
    try:
        ...
        # 检查权限：系统管理员、管理员、区域护林员和监管人员可以访问设备状态API
        if role not in ['系统管理员', '管理员', '区域护林员', '监管人员']:
            return jsonify({'error': '没有权限访问该API'}), 403
...
def add_equipment_inspection():
    try:
        ...
        # 检查权限：系统管理员、管理员、区域护林员和监管人员可以访问设备巡检API
        if role not in ['系统管理员', '管理员', '区域护林员', '监管人员']:
            return jsonify({'error': '没有权限访问该API'}), 403
...
@app.route('/api/equipment_management/audit_equipment_inspection', methods=['POST'])
def audit_equipment_inspection():
    try:
        if 'user' not in session:
            return jsonify({'error': '未登录'}), 401
        ...
        # 检查权限：只有监管人员可以审核设备维护记录
        if role != '监管人员':
            return jsonify({'error': '没有权限审核设备维护记录', 'success': False}), 403
        
        # 获取请求数据
        audit_data = request.get_json()
        
        inspection_id = audit_data.get('inspection_id')
        audit_status = audit_data.get('audit_status')
        audit_comments = audit_data.get('audit_comments')
        
        if not inspection_id or not audit_status:
            return jsonify({'error': '缺少必要参数', 'success': False}), 400
        
        # 调用业务层审核设备维护记录
        success = db_instance.audit_equipment_inspection(inspection_id, audit_status, user_id, audit_comments)
        return jsonify({'success': success})
    except Exception as e:
        print(f"审核设备维护记录时发生异常: {e}")
        import traceback
        traceback.print_exc()
        return jsonify({'success': False, 'error': str(e)})
...
def get_equipment_inspections():
    try:
        ...
        # 检查权限：系统管理员、区域护林员和监管人员可以访问设备巡检API
        if role not in ['系统管理员', '区域护林员', '监管人员']:
            return jsonify({'error': '没有权限访问该API'}), 403
...
def get_report_templates():
    ...
    # 检查权限：只有系统管理员、数据管理员和监管人员可以访问报表模板API
    role = session['user']['Role']
    if role not in ['系统管理员', '数据管理员']:
        return jsonify({'error': '没有权限访问该API'}), 403
    ...
    # 数据管理员只能看到审核通过的模板，系统管理员可以看到所有模板
    if role == '数据管理员' and not audit_status:
        audit_status = '已通过'
    
    templates = db_instance.get_report_templates(report_type, audit_status, is_active_bool)
    return jsonify(templates)

@app.route('/api/statistical_analysis/audit_report_template', methods=['POST'])
def audit_report_template():
    """
    审核报表模板
    """
    if 'user' not in session:
    ...
    # 检查权限：只有系统管理员可以审核报表模板
    if role != '系统管理员':
        return jsonify({'error': '没有权限审核报表模板'}), 403
    ...
    try:
        ...
        if not template_id or not audit_status:
            return jsonify({'error': '缺少必要参数'}), 400
        
        if audit_status not in ['已通过', '已拒绝']:
            return jsonify({'error': '无效的审核状态'}), 400
        
        # 执行审核
        if db_instance.audit_report_template(template_id, audit_status, user_id, audit_comments):
            return jsonify({'success': True, 'message': '报表模板审核成功'})
        else:
            return jsonify({'error': '报表模板审核失败'}), 500
    except Exception as e:
        print(f"审核报表模板失败: {e}")
        return jsonify({'error': str(e)}), 500

@app.route('/api/statistical_analysis/update_template_status', methods=['POST'])
def update_template_status():
    ...
    # 检查权限：只有系统管理员可以更新模板状态
    if role != '系统管理员':
        return jsonify({'error': '没有权限更新模板状态'}), 403
    ...
    try:
        ...
        if template_id is None or is_active is None:
            return jsonify({'error': '缺少必要参数'}), 400
        
        # 执行状态更新
        if db_instance.update_report_template_status(template_id, is_active):
            return jsonify({'success': True, 'message': '报表模板状态更新成功'})
        else:
            return jsonify({'error': '报表模板状态更新失败'}), 500
    except Exception as e:
        print(f"更新报表模板状态失败: {e}")
        return jsonify({'error': str(e)}), 500

@app.route('/api/statistical_analysis/generate_report', methods=['POST'])
def generate_report():
    if 'user' not in session:
        return jsonify({'error': '未登录'}), 401
    ...
    role = session['user']['Role']
    if role != '数据管理员':
        return jsonify({'error': '没有权限生成报表'}), 403
    ...
    try:
        ...
        if not template_id or not region_id or not start_time or not end_time:
            return jsonify({'error': '缺少必要参数'}), 400
        ...
        # 生成报表
        report_id = db_instance.generate_report_from_template(
            template_id, region_id, start_time_obj, end_time_obj, session['user']['UserID']
        )
        
        if report_id:
            return jsonify({'success': True, 'message': '统计报表生成成功', 'report_id': report_id})
        else:
            return jsonify({'error': '报表生成失败'}), 500
    except Exception as e:
        print(f"生成报表失败: {e}")
        return jsonify({'error': '报表生成失败'}), 500

@app.route('/api/statistical_analysis/publish_report', methods=['POST'])
def publish_report():
    if 'user' not in session:
        return jsonify({'error': '未登录'}), 401
    ...
    role = session['user']['Role']
    if role != '数据管理员':
        return jsonify({'error': '没有权限发布报表'}), 403
    
    data = request.get_json()
    try:
        report_id = data.get('report_id')
        if not report_id:
            return jsonify({'error': '缺少报表ID'}), 400
        
        # 更新报表状态为已发布（使用现有的ReportName列，在名称前添加[已发布]标识）
        # 查询当前报表名称
        current_name = db_instance.db.fetch_one("SELECT ReportName FROM GeneratedReport WHERE ReportID = ?", (report_id,))
        if current_name:
            current_report_name = current_name[0]
            # 如果还没有添加[已发布]标识，则添加
            if not current_report_name.startswith('[已发布]'):
                new_report_name = f"[已发布]{current_report_name}"
                sql = "UPDATE GeneratedReport SET ReportName = ? WHERE ReportID = ?"
                if db_instance.db.execute(sql, (new_report_name, report_id)):
                    return jsonify({'success': True, 'message': '统计报表发布成功'})
            else:
                # 已经是已发布状态
                return jsonify({'success': True, 'message': '统计报表已经是已发布状态'})
        return jsonify({'error': '报表发布失败'}), 500
    except Exception as e:
        print(f"发布报表失败: {e}")
        return jsonify({'error': str(e)}), 500
...
# 反馈API - 获取反馈列表（仅限系统管理员和数据管理员）
@app.route('/api/feedback/get_feedbacks')
def get_feedbacks():
    if 'user' not in session:
        return jsonify({'error': '未登录'}), 401
    
    # 检查权限：只有系统管理员和监管人员可以访问反馈列表
    user = session['user']
    role = user['Role']
    user_id = user['UserID']
...
def get_generated_reports():
    ...
    if role == '数据管理员':
        ...
        # 系统管理员只能查看已发布报表的名称和详情
        all_reports = db_instance.get_generated_reports(region_id, report_type, start_time, end_time)
        reports = []
