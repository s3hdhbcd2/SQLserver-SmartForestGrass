...
def get_menu():
    ...
    # 统一使用系统管理员角色 - 系统管理员不需要环境监测和资源管理
    if username == 'admin' or role in ['系统管理员', 'admin', 'system_admin']:
        menu = [
...
def equipment_management():
    ...
    # 检查权限：只有系统管理员、区域护林员和监管人员可以访问设备管理
    role = session['user']['Role']
...
def get_regions():
    ...
    # 检查权限：系统管理员、数据管理员、区域护林员和监管人员可以访问区域列表API
    if role not in ['系统管理员', '数据管理员', '区域护林员', '监管人员']:
        return jsonify({'error': '没有权限访问该API', 'success': False}), 403
...
def get_devices():
    try:
        ...
        # 检查权限：只有系统管理员、区域护林员和监管人员可以访问设备管理API
        if role not in ['系统管理员', '区域护林员', '监管人员']:
...
def get_device_status():
    try:
        ...
        # 检查权限：只有区域护林员和监管人员可以访问设备状态API（系统管理员不需要设备状态查询）
...
def add_equipment_inspection():
    try:
        ...
        # 检查权限：只有区域护林员和监管人员可以访问设备巡检API（系统管理员不需要设备巡检功能）
        if role not in ['区域护林员', '监管人员']:
            return jsonify({'error': '没有权限访问该API'}), 403
...
def update_device():
    ...
    # 检查权限：只有系统管理员、区域护林员可以访问设备管理API
    if role not in ['系统管理员', '数据管理员', '监管人员']:
        return jsonify({'error': '没有权限访问该API'}), 403
