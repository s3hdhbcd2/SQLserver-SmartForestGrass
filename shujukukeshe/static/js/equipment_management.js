...
function loadMaintenanceRecords() {
    const deviceId = document.getElementById('maintenance-device-id-select').value;
    const tableBody = document.getElementById('maintenance-records-table');
    tableBody.innerHTML = '<tr><td colspan="9" style="text-align: center; color: #999;">数据加载中...</td></tr>';
    
    if (!deviceId) {
        tableBody.innerHTML = '<tr><td colspan="9" style="text-align: center; color: #999;">请选择设备ID</td></tr>';
        return;
    }
    ...
    const url = `/api/equipment_management/get_equipment_inspections?device_id=${deviceId}`;
    ...
    fetch(url)
        ...
        .then(data => {
            ...
            if (!Array.isArray(data) || data.length === 0) {
                tableBody.innerHTML = '<tr><td colspan="9" style="text-align: center; color: #999;">未找到匹配的设备维护记录</td></tr>';
                return;
            }
        ...
        .catch(error => {
            ...
            tableBody.innerHTML = '<tr><td colspan="9" style="text-align: center; color: #e74c3c;">数据加载失败，请刷新页面重试</td></tr>';
        });
}
