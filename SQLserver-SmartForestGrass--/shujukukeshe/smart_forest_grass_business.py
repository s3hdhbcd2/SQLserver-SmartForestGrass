                ...
                if resource_choice == '1':
                    ...
                    if resources:
                        for resource in resources:
                            print(f"资源ID: {resource['ResourceID']}, 区域ID: {resource['region_id']}, 类型: {resource['ResourceType']}, 面积: {resource['CoverageArea']}")
                    else:
                        print("暂无林草资源数据")
                ...
                if equipment_choice == '1':
                    ...
                    if devices:
                        for device in devices:
                            print(f"设备ID: {device['DeviceID']}, 名称: {device['DeviceName']}, 类型: {device['DeviceType']}, 区域ID: {device['region_id']}")
                    else:
                        print("暂无设备档案数据")
...
class DisasterWarningDatabase:
    def __init__(self, db_connection):
        ...
        self.env_module = EnvironmentalMonitoringModule(db_connection)
        self.warning_module = DisasterWarningModule(db_connection)
        self.resource_module = ResourceManagementModule(db_connection)
        self.equipment_module = EquipmentManagementModule(db_connection)
        self.statistics_module = StatisticalAnalysisModule(db_connection)
    ...
    def get_resource_change_records(self, resource_id=None):
        ...
        :return: 资源变动记录列表
        """
        return self.resource_module.get_resource_change_records(resource_id)
    ...
    def get_device_archives(self, region_id=None, device_type=None):
        ...
        :return: 设备档案列表
        """
        return self.equipment_module.get_device_archives(region_id, device_type)
    ...
    def get_report_templates(self, report_type=None):
        ...
        :return: 报表模板列表
        """
        return self.statistics_module.get_report_templates(report_type)
